...
|---|-------------|------------|-------------|
| 1 | Generic01   | `NULL`     | Empty nodes list. |
| 2 | Generic02   | `EMPTY`    | Empty nodes list. |
...
| # | Test method   | Hex Data         | Description |
|---|---------------|------------------|-------------|
| 1 | TLV_Invalid01 | `01`             | Missing length and value. |
| 2 | TLV_Invalid02 | `9F 1A 02 12`    | Declared length 2 but only one value byte. |
| 3 | TLV_Invalid03 | `5F`             | Single-octet tag marker 5F requires a following tag octet. |
| 4 | TLV_Invalid04 | `04 81`          | Length octet 81 indicates one subsequent length byte, but length byte and value are missing. |
| 5 | TLV_Invalid05 | `DF 00 05 AA BB` | Declared length 5 but only 2 value bytes provided. |
